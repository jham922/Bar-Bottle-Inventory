...
| `app/(app)/inventory/index.tsx` | Inventory list screen — reloads on focus via `useFocusEffect` |
| `app/(app)/inventory/[id].tsx` | Bottle detail + delete — uses `.maybeSingle()` not `.single()` |
| `app/(app)/scan/single.tsx` | Single bottle scan + confirm screen (name, %, size all editable) |
...
- **`.single()` vs `.maybeSingle()`** — always use `.maybeSingle()` for queries that might return 0 rows. `.single()` throws a PostgREST error ("Cannot coerce the result to a single JSON object") when no rows are found.
- **Inventory list stale after delete** — use `useFocusEffect` to reload list whenever the screen comes into focus.
- **CRLF on Windows** — Git may warn about LF→CRLF on `dist/` files. This is harmless.
